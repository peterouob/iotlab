...
char masterMessage[] = "Ping from Master"; 
...
void send_cb(void) {
  Serial.println("Master message sent. Listening for reply...");
  api.lorawan.precv(2000);
}
...
void setup() {
  ...
  Serial.println("RAK P2P Master Node");
  Serial.println("------------------------------------------------------");
  ...
  Serial.println("Setup Complete. Starting auto-send loop.");
}
...
void loop() {
  if (millis() - lastTxTime > txInterval) {
    ...
    Serial.printf("\n>>> Sending Master Ping <<<\n");
    bool send_result = api.lorawan.psend(sizeof(masterMessage) - 1, (uint8_t*)masterMessage);
    if (!send_result) {
      Serial.println("P2P send command FAILED");
    }
  }
}